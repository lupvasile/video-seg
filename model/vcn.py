...
class VCN(nn.Module):
    ...
    def forward(self, im, disc_aux=None):
        ...
        ## matching 5
        up_flow6 = F.interpolate(flow6, [im.size()[2] // 32, im.size()[3] // 32], mode='bilinear') * 2
        ...
        flow5h = torch.cat((flow5h, F.interpolate(flow6h.detach() * 2, [flow5h.shape[2], flow5h.shape[3]], mode='bilinear')), 1)  # b, k2--k2, h, w
        ent5h = torch.cat((ent5h, F.interpolate(ent6h, [flow5h.shape[2], flow5h.shape[3]], mode='bilinear')), 1)
        ...
        ## matching 4
        up_flow5 = F.interpolate(flow5, [im.size()[2] // 16, im.size()[3] // 16], mode='bilinear') * 2
        ...
        # append coarse hypotheses
        flow4h = torch.cat((flow4h, F.interpolate(flow5h.detach() * 2, [flow4h.shape[2], flow4h.shape[3]], mode='bilinear')), 1)
        ent4h = torch.cat((ent4h, F.interpolate(ent5h, [flow4h.shape[2], flow4h.shape[3]], mode='bilinear')), 1)
        ...
        ## matching 3
        up_flow4 = F.interpolate(flow4, [im.size()[2] // 8, im.size()[3] // 8], mode='bilinear') * 2
        ...
        # append coarse hypotheses
        flow3h = torch.cat((flow3h, F.interpolate(flow4h.detach() * 2, [flow3h.shape[2], flow3h.shape[3]], mode='bilinear')), 1)
        ent3h = torch.cat((ent3h, F.interpolate(ent4h, [flow3h.shape[2], flow3h.shape[3]], mode='bilinear')), 1)
        ...
        ## matching 2
        up_flow3 = F.interpolate(flow3, [im.size()[2] // 4, im.size()[3] // 4], mode='bilinear') * 2
        ...
        # append coarse hypotheses
        flow2h = torch.cat((flow2h, F.interpolate(flow3h.detach() * 2, [flow2h.shape[2], flow2h.shape[3]], mode='bilinear')), 1)
        ent2h = torch.cat((ent2h, F.interpolate(ent3h, [flow2h.shape[2], flow2h.shape[3]], mode='bilinear')), 1)
        ...
        flow2 = F.interpolate(flow2, [im.size()[2], im.size()[3]], mode='bilinear')
        flow3 = F.interpolate(flow3, [im.size()[2], im.size()[3]], mode='bilinear')
        flow4 = F.interpolate(flow4, [im.size()[2], im.size()[3]], mode='bilinear')
        flow5 = F.interpolate(flow5, [im.size()[2], im.size()[3]], mode='bilinear')
        flow6 = F.interpolate(flow6, [im.size()[2], im.size()[3]], mode='bilinear')
        ...
        if self.training:
            ...
            up_flow3 = F.interpolate(up_flow3, [im.size()[2], im.size()[3]], mode='bilinear') * 4
            up_flow4 = F.interpolate(up_flow4, [im.size()[2], im.size()[3]], mode='bilinear') * 8
            up_flow5 = F.interpolate(up_flow5, [im.size()[2], im.size()[3]], mode='bilinear') * 16
            up_flow6 = F.interpolate(up_flow6, [im.size()[2], im.size()[3]], mode='bilinear') * 32
            oor2 = F.interpolate(oor2[:, np.newaxis], [im.size()[2], im.size()[3]], mode='bilinear')[:, 0]
            oor3 = F.interpolate(oor3[:, np.newaxis], [im.size()[2], im.size()[3]], mode='bilinear')[:, 0]
            oor4 = F.interpolate(oor4[:, np.newaxis], [im.size()[2], im.size()[3]], mode='bilinear')[:, 0]
            oor5 = F.interpolate(oor5[:, np.newaxis], [im.size()[2], im.size()[3]], mode='bilinear')[:, 0]
            oor6 = F.interpolate(oor6[:, np.newaxis], [im.size()[2], im.size()[3]], mode='bilinear')[:, 0]
